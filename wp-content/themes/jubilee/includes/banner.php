 ...
 */
function jubilee_banner_related_pages() {
	...
	return apply_filters( 'jubilee_banner_pages', $pages_query->posts );
 ...
 * Banner data
 *
 * Get data for use with building the banner.
 *
 * @since 1.0
 * @global object Post object
 * @return array Banner data
 */
function jubilee_banner_data() {
	...
	// Priority 1: Page or post of any type (except person and attachment) with featured image is being shown -- use that
	if ( ( is_page() || is_single() ) && ! is_singular( 'ctc_person' ) && has_post_thumbnail() ) {
	...
	// Priority 5: Not on page or post with its own featured image -- use related page's featured image
	if ( empty( $data['page'] ) ) {
	...
	// Return filtered
	return apply_filters( 'jubilee_banner_data', $data );

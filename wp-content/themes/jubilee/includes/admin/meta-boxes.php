 ...
 * @since 1.0
 */
function jubilee_add_meta_box_options() {
	...
	$banner_brightness_options = array(
		...
		'' => sprintf(
			__( 'Customizer Setting (%s%%)', 'jubilee' ),
			ctfw_customization( 'header_image_brightness' )
		)
	);
	for ( $i = 1; $i <= 100; $i++ ) {
		$banner_brightness_options[$i] = sprintf( _x( '%d%%', 'banner image brightness', 'jubilee' ), $i );
	}

	// Banner image colorization options
	$banner_colorization_options = array(
		/* translators: %s is Customizer header colorization value. '%%' represents '%' (use two characters) */
		'' => sprintf(
			__( 'Customizer Setting (%s%%)', 'jubilee' ),
			ctfw_customization( 'colorization' )
		)
	);
	for ( $i = 1; $i <= 100; $i++ ) {
		$banner_colorization_options[$i] = sprintf( _x( '%d%%', 'banner image colorization', 'jubilee' ), $i );
	}
	...
	foreach ( $post_types as $post_type ) {
		...
		$meta_box = array(
			...
			// Meta Box
			'id' 		=> 'jubilee_options', // unique ID
			'title' 	=> __( 'Options', 'jubilee' ),
			'post_type'	=> $post_type,
		...
		if ( 'page' == $post_type ) {
			...
			$meta_box['fields']['_ctcom_banner_related'] = array( // using _ctcom_ prefix for portability between themes
				'name'				=> __( 'Banner on Related Content', 'jubilee' ),
				'after_name'		=> '', // (Optional), (Required), etc.
				'desc'				=> __( "Example: Sermons template used. This page's banner shows on all sermon pages.", 'jubilee' ),
				'type'				=> 'checkbox', // text, textarea, checkbox, checkbox_multiple, radio, select, number, upload, upload_textarea, url
				'checkbox_label'	=> __( "Show banner on related content", 'jubilee' ), //show text after checkbox
				...
				'date_multiple'		=> false, // whether or not to allow date field type to select multiple dates, to be jubilee as comma-separated list.
				'date_button'       => '', // text for button user clicks to open datepicker calendar.
				'default'			=> '', // value to pre-populate option with (before first save or on reset)
				'no_empty'			=> false, // if user empties value, force default to be jubilee instead
				...
				'custom_sanitize'	=> 'jubilee_sanitize_banner_related', // function to do additional sanitization
		...
		$meta_box['fields']['_ctcom_banner_image_brightness'] = array( // using _ctcom_ prefix for portability between themes
			'name'				=> _x( 'Banner Image Brightness', 'meta box', 'jubilee' ),
			'after_name'		=> '', // (Optional), (Required), etc.
			'desc'				=> _x( 'Lower percentage makes image darker. Aim for easy to read title.', 'banner custom field', 'jubilee' ),
		...
		// Banner Colorization Override
		$meta_box['fields']['_ctcom_banner_colorization'] = array( // using _ctcom_ prefix for portability between themes
			'name'				=> _x( 'Banner Colorization', 'meta box', 'jubilee' ),
			'after_name'		=> '', // (Optional), (Required), etc.
			'desc'				=> _x( 'Higher percentage increases color. Aim for easy to read title.', 'banner custom field', 'jubilee' ),
			'type'				=> 'select', // text, textarea, checkbox, checkbox_multiple, radio, select, number, upload, upload_textarea, url
			'checkbox_label'	=> '', //show text after checkbox
			'options'			=> $banner_colorization_options, // array of keys/values for radio or select
			'upload_button'		=> '', // text for button that opens media frame
			'upload_title'		=> '', // title appearing at top of media frame
			'upload_type'		=> '', // optional type of media to filter by (image, audio, video, application/pdf)
			'date_multiple'		=> false, // whether or not to allow date field type to select multiple dates, to be jubilee as comma-separated list.
		...
		$meta_box['fields']['_ctcom_gallery_exclude'] = array( // using _ctcom_ prefix for portability between themes
			'name'				=> _x( 'Gallery Content', 'meta box', 'jubilee' ),
			'after_name'		=> '', // (Optional), (Required), etc.
			'desc'				=> '',
			'type'				=> 'checkbox', // text, textarea, checkbox, checkbox_multiple, radio, select, number, upload, upload_textarea, url
			'checkbox_label'	=> __( 'Exclude gallery content from gallery widgets and templates', 'jubilee' ), //show text after checkbox
			'options'			=> array(), // array of keys/values for radio or select
			'upload_button'		=> '', // text for button that opens media frame
			'upload_title'		=> '', // title appearing at top of media frame
			'upload_type'		=> '', // optional type of media to filter by (image, audio, video, application/pdf)
...
add_action( 'admin_init', 'jubilee_add_meta_box_options' );
...
function jubilee_sanitize_banner_related( $value ) {
		...
		if ( ! empty( $page_template ) && 'default' != $page_template ) {
			...
			if ( $content_type ) {
				...
				if ( ! empty( $content_type_page_templates ) ) {
					...
					$banner_pages = jubilee_banner_related_pages();

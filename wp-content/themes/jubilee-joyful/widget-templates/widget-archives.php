...
if ( $archives ) {
	...
	foreach ( $archives as $archive ) {
		...
		$text = sprintf( _x('%1$s %2$d', 'month archive', 'jubilee' ), $wp_locale->get_month( $archive->month ), $archive->year );
		...
		$items .= get_archives_link( $archive->url, $text, $format, '', $after );
	...
	if ( ! empty( $instance['show_dropdown'] ) ) {
		...
		echo '		<option value="">' . _x( 'Select Month', 'archives widget', 'jubilee' ) . '</option>';
		echo $items;
		echo '	</select>';
		echo '</form>';
	}

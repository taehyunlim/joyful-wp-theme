...
function jubilee_calendar_after_content() {
	...
	$month_name = date_i18n( _x( 'F Y', 'event calendar', 'jubilee' ), $month_ts );
	...
	<section id="jubilee-calendar">
			...
			<div id="jubilee-calendar-header">
							...
							<span id="jubilee-calendar-title-category">
								...
								<?php if ( $category_chosen ) : ?>
									<span id="jubilee-calendar-remove-category">
										...
										?>" class="<?php jubilee_icon_class( 'calendar-remove' ); ?> jubilee-calendar-control" title="<?php echo esc_attr( _x( 'Remove Category Filter', 'event calendar', 'jubilee' ) ); ?>"></a>
									</span>
								<?php endif; ?>
				...
				<ul id="jubilee-calendar-header-right" class="jubilee-list-icons">
					...
					<?php if ( $show_prev_button ) : ?>
						<li><a href="<?php echo esc_url( remove_query_arg( '_pjax', add_query_arg( 'month', $prev_month ) ) ); ?>" class="jubilee-calendar-control <?php jubilee_icon_class( 'calendar-prev' ); ?><?php if ( ! $show_next_button ) : ?> jubilee-calendar-no-next<?php endif; ?>" title="<?php echo esc_attr( sprintf( __( 'Previous Month &ndash; %1$s', 'jubilee' ), date_i18n( 'F', $prev_month_ts ) ) ); ?>"></a></li>
					<?php endif; ?>

					<?php if ( $show_next_button ) : ?>
						<li><a href="<?php echo esc_url( remove_query_arg( '_pjax', add_query_arg( 'month', $next_month ) ) ); ?>" class="jubilee-calendar-control <?php jubilee_icon_class( 'calendar-next' ); ?><?php if ( ! $show_prev_button ) : ?> jubilee-calendar-no-prev<?php endif; ?>" title="<?php echo esc_attr( sprintf( __( 'Next Month &ndash; %1$s', 'jubilee' ), date_i18n( 'F', $next_month_ts ) ) ); ?>"></a></li>
					<?php endif; ?>

					<li><a href="#" id="jubilee-calendar-month-control" class="jubilee-calendar-dropdown-control <?php jubilee_icon_class( 'calendar-month' ); ?>" title="<?php echo esc_attr( _x( 'Choose Month', 'event calendar', 'jubilee' ) ); ?>"></a></li>

					<?php if ( $categories_enabled ) : ?>
						<li><a href="#" id="jubilee-calendar-category-control" class="jubilee-calendar-dropdown-control <?php jubilee_icon_class( 'calendar-category' ); ?>" title="<?php echo esc_attr( _x( 'Choose Category', 'event calendar', 'jubilee' ) ); ?>"></a></li>
					<?php endif; ?>
			...
			<table id="jubilee-calendar-table">
					...
					<?php foreach ( $calendar['days_of_week'] as $day_of_week ) : ?>
						...
						<th class="jubilee-calendar-table-header">
							<div class="jubilee-calendar-table-header-content">
								<span class="jubilee-calendar-table-header-full"><?php echo $day_of_week['name']; ?></span>
								<span class="jubilee-calendar-table-header-short"><?php echo $day_of_week['name_short']; ?></span>
							</div>
						</th>
				...
				<?php foreach ( $calendar['weeks'] as $week_of_month => $week ) : ?>
					...
					<tr class="<?php
							...
							$week_classes = array();
							$week_classes[] = 'jubilee-calendar-table-week';

							// First week
							if ( 0 == $week_of_month ) {
								$week_classes[] = 'jubilee-calendar-table-week-first';
							}

							// Last week
							if ( $weeks_in_month == ( $week_of_month + 1 ) ) {
								$week_classes[] = 'jubilee-calendar-table-week-last';
							}
						...
						<?php foreach ( $week['days'] as $day_of_week => $day ) : ?>
							...
							$day_classes = array();
							$day_classes[] = 'jubilee-calendar-table-day';

							// Day of week (1 - 7)
							$day_classes[] = 'jubilee-calendar-table-day-' . $day_of_week_friendly;

							// Day is in previous or next month
							if ( $day['other_month'] ) {
								$day_classes[] = 'jubilee-calendar-table-day-other-month';
							}

							// Day is today
							if ( $is_today ) {
								$day_classes[] = 'jubilee-calendar-table-day-today';
							}

							// Day is yesterday
							if ( $is_yesterday ) {
								$day_classes[] = 'jubilee-calendar-table-day-yesterday';
							}

							// Day is tomorrow
							if ( $is_tomorrow ) {
								$day_classes[] = 'jubilee-calendar-table-day-tomorrow';
							}

							// Day is previous week
							if ( $is_day_previous_week ) {
								$day_classes[] = 'jubilee-calendar-table-day-previous-week';
							}

							// Day is next week
							if ( $is_day_next_week ) {
								$day_classes[] = 'jubilee-calendar-table-day-next-week';
							}

							// Day is last day of week
							if ( $is_last_day_of_week ) {
								$day_classes[] = 'jubilee-calendar-table-day-last-of-week';
							}

							// Day is in past
							if ( $day['date_ts'] < $today_ts ) {
								$day_classes[] = 'jubilee-calendar-table-day-past';
							}

							// Has events
							if ( ! empty( $day['event_ids'] ) ) {
								$day_classes[] = 'jubilee-calendar-table-day-has-events';
							}
									...
									<div class="jubilee-calendar-table-day-content">
										...
										<div class="jubilee-calendar-table-day-heading">
											...
											<?php if ( $day['first_of_next_month'] || $day['last_of_previous_month'] ) : ?>
												<span class="jubilee-calendar-table-day-label"><?php echo date_i18n( 'M', $day['date_ts'] ); ?></span>
											<?php elseif ( $is_today ) : ?>
												<span class="jubilee-calendar-table-day-label"><?php _ex( 'Today', 'event calendar', 'jubilee' ); ?></span>
											<?php endif; ?>
												...
												<?php foreach ( $day['event_ids'] as $event_id ) : ?>
													...
													<li data-event-id="<?php the_ID(); ?>">
														...
														<?php if ( $time_range_or_description ) : ?>
															<span class="jubilee-calendar-table-day-event-time"><?php echo esc_html( $time_range_or_description ); ?></span>
														<?php endif; ?>
			...
			<div id="jubilee-calendar-month-dropdown" class="jq-dropdown jubilee-calendar-dropdown jq-dropdown-anchor-right">
		  		...
		  		<div class="jq-dropdown-panel">
					...
					?>
			  			...
			  			<div class="jubilee-calendar-month-dropdown-year"><?php echo $year; ?></div>
			  			...
			  			<ul class="jubilee-calendar-month-dropdown-months">
				  			...
				  			<?php for ( $month = 1; $month <= 12; $month ++ ) : ?>
				  				...
				  				<li<?php
					  				...
					  				// Month is in past
				  					if ( $month_is_past ) {
					  					$classes[] = 'jubilee-calendar-month-dropdown-past';
				  					}
					...
					<ul class="jq-dropdown-menu">
						...
						<li<?php if ( empty( $category ) ) : ?> class="jq-dropdown-selected"<?php endif; ?>>
							<a href="<?php echo esc_url( remove_query_arg( array( 'category', '_pjax' ) ) ); ?>">
								<?php _ex( 'All Categories', 'event calendar', 'jubilee' ); ?>
							</a>
						</li>
		...
		<section id="jubilee-calendar-list">
			...
			<h3 id="jubilee-calendar-list-heading" class="screen-reader-text"><?php _ex( 'Events', 'calendar', 'jubilee' ); ?></h3>
...
add_action( 'jubilee_after_content', 'jubilee_calendar_after_content' );
